...
<div class="container-fluid">
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        if(empty($err)){
           ...
           $pict=rand(0,100000)."_".$pictName;
            ...
            $stmt=$con->prepare("update posts set subject=?,content=?,service=?,image=?,date=now(),admin_id=? where id=?");
            $stmt->execute(array($_POST['subject'],$_POST['content'],$_POST['service'],$pict,$_SESSION['id'],$_POST['id']));
               if($stmt){
                      ?>
 ...
 ?>  
   ...
   <div class="form-group">
    <div class="col-md-7  col-sm-9">
        <input type="text" class="form-control" id="sub" name="subject" placeholder="ادخل العنوان" required oninvalid="this.setCustomValidity('الرجاء ادخال العنوان')" oninput="setCustomValidity('')" value="<?php echo $r['subject']?>">
    </div>
   ...
   <div class="form-group">
    <div class="col-md-7  col-sm-9">
        ...
        required oninvalid="this.setCustomValidity('الرجاء ادخال المحتوى')" oninput="setCustomValidity('')"><?php echo $r['subject']?></textarea>
    </div>
    ...
    <div class="form-group">
    <div class="col-md-7  col-sm-9">
        <p style='direction:rtl;font-size:18px;'>اذا لم يتم اختيار صورة سوف تاخذ الصورة القديمة</p>
  ...
  <div class="form-group">
    <div class="col-md-7  col-sm-9">
         <select class="form-control" id="ser" name='service' style="font-size: 22px;height: 45px; " required>
            ...
            <option  value='e' <?php if($r['service']=='e'){echo "selected";} ?>>كهرباء</option>
            <option  value='g' <?php if($r['service']=='g'){echo "selected";} ?>>غاز</option>
            <option  value='w' <?php if($r['service']=='w'){echo "selected";} ?>>مياه</option>
         </select>
    </div>
