...
<div class="container-fluid">
   ...
   <h2>تعديل بيانات المسؤول</h2>    
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        if(empty($_POST['password'])){
            $err[]='حقل الرقم السري فارغ' ;
        }
        if(strlen($password) < 3)
        {
            $err[] = "كلمة المرور يجب الإ تقل عن 3 احرف";
        }
        ...
        if(!filter_var($_POST['phone'], FILTER_VALIDATE_INT)){
            $err[]='اادخل رقم تليفون صحيح, ارقام فقط' ;
        }
        if(empty($_POST['name'])){
        ...
        if(empty($err)){
            ...
            $stmt=$con->prepare("update admin set UserName=?,Password=?,Phone=?,FullName=?,Email=? where Admin_id=?");
                      $stmt->execute(array($_POST['admin'],$_POST['password'],$_POST['phone'],$_POST['name'],$_POST['email'],$_POST['id']));
                      if($stmt){
                          ?>
 ...
 ?>  
  ...
  <div class="form-group">
    <div class="col-md-7  col-sm-9">
      <input type="text" class="form-control" id="admin" name="admin" placeholder=" اسم المسؤل" value="<?php echo $row['UserName']?>" required oninvalid="this.setCustomValidity('الرجاء ادخال اسم المسؤول')" oninput="setCustomValidity('')">
  ...
  <div class="form-group">
    <div class="col-md-7  col-sm-9">
      <input type="password" class="form-control prob"  id="password" name="password" placeholder=" كلمة المرور" value="<?php echo $row['Password']?>" required oninvalid="this.setCustomValidity('الرجاء ادخال كلمة المرور')" oninput="setCustomValidity('')">
    </div>
    ...
    <div class="form-group">
    <div class="col-md-7  col-sm-9">
      <input type="password" class="form-control prob"  id="repassword" name="repassword" placeholder=" تاكيد كلمة المرور" value="<?php echo $row['Password']?>" required oninvalid="this.setCustomValidity('الرجاء ادخال تاكيد كلمة المرور')" oninput="setCustomValidity('')">
    </div>
  ...
  <div class="form-group">
    <div class="col-md-7  col-sm-9">
      <input type="text" class="form-control" id="phone" name="phone" placeholder="رقم التليفون" value="<?php echo $row['Phone']?>" required oninvalid="this.setCustomValidity('الرجاء ادخال رقم التليفون')" oninput="setCustomValidity('')">
    </div>
    ...
    <div class="col-md-3 col-sm-4 hide"></div>
        ...
        <div class="form-group">
    <div class="col-md-7  col-sm-9">
      <input type="text" class="form-control" id="name" name="name" placeholder="اسم المسؤول بالكامل" value="<?php echo $row['FullName']?>" required oninvalid="this.setCustomValidity('الرجاء ادخال اسم المسؤول بالكامل')" oninput="setCustomValidity('')">
    </div>
    <label class="control-label col-md-2 col-sm-3" for="name"> : اسم المسؤل بالكامل</label>
    <div class="col-md-3 col-sm-4 hide"></div>
  </div>
        <div class="form-group">
    <div class="col-md-7  col-sm-9">
      <input type="email" class="form-control prob" id="email" name="email" placeholder="الايميل" value="<?php echo $row['Email']?>" required oninvalid="this.setCustomValidity('الرجاء ادخال الايميل')" oninput="setCustomValidity('')">
    </div>
